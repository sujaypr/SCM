...
class ForecastRequestSchema(Schema):
    ...
    businessType = fields.Str(
        ...
        error_messages={'invalid_choice': 'Invalid business type. Please select from available options.'}
    )

    businessScale = fields.Str(
        required=True,
        validate=validate.OneOf(['Micro', 'Small', 'Medium']),
        error_messages={'invalid_choice': 'Business scale must be Micro, Small, or Medium (MSME classification).'}
    )
...
class InventoryItemSchema(Schema):
    ...
    category = fields.Str(
        required=True,
        validate=validate.OneOf([
            'Grocery', 'Electronics', 'Clothing', 'Medical',
            'Cosmetics', 'Food & Beverage', 'Books', 'Sports',
            'Home & Garden', 'Automotive', 'Other'
        ])
    )
...
class ReportRequestSchema(Schema):
    ...
    report_type = fields.Str(
        required=True,
        validate=validate.OneOf([
            'executive-summary', 'sales', 'inventory', 
            'forecast-accuracy', 'logistics'
        ]),
        error_messages={
...
def validate_indian_state(state_name):
    ...
    return state_name in indian_states or any(state.lower() in state_name.lower() for state in indian_states)
...
def validate_phone_number_format(phone):
    ...
    return re.match(phone_pattern, clean_phone) or re.match(landline_pattern, clean_phone)